...
const LoginModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      document.cookie = `TOKEN=${response.data.content.token}; path=/; samesite=lax`;
  ...
  return (
    <section
      className="app-container mx-auto sm:max-w-md md:max-w-lg lg:max-w-md xl:max-w-lg bg-white rounded-2xl p-5 sm:p-6 md:p-8 lg:p-8 xl:p-10 relative shadow-lg"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        onClick={onClose}
        className="absolute right-4 top-4 text-[#272B45] hover:text-[#ED1B24]  transition cursor-pointer"
      >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          />
          {errors.email && (
            <p className="mt-1 text-sm text-[#ED1B24] ">{errors.email}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.password && (
            <p className="mt-1 text-sm text-[#ED1B24] ">{errors.password}</p>
          )}
        </div>
