...
import {
    ...
    CheckCircle2, Banknote, Landmark, Award, X, AlertCircle, Loader2
} from 'lucide-react';
...
const CheckoutPage = () => {
    ...
    const maxGuests = React.useMemo(() => {
        return Number(checkoutData?.guests) || 1;
    }, [checkoutData]);
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-4xl p-8 shadow-sm border border-slate-200 divide-y divide-slate-100">
                                <div className="flex justify-between items-center pb-6">
                                    ...
                                    <button onClick={() => setIsCalendarOpen(true)} className="text-sm font-bold underline underline-offset-4 hover:text-rose-600 duration-300 transition-all cursor-pointer">Edit</button>
                                </div>
                                <div className="flex justify-between items-center pt-6">
                                    ...
                                    <div className="flex items-center gap-4 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                                        <button
                                            ...
                                            disabled={guests <= 1}
                                            className="w-8 h-8 flex items-center justify-center hover:bg-white hover:shadow-sm rounded-lg duration-300 transition-all font-bold cursor-pointer">-</button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#143944] /60 backdrop-blur-md p-4">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-200">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setIsCalendarOpen(false)} className="p-2 hover:bg-slate-100 rounded-full duration-300 transition-all border border-slate-100"><X size={20} /></button>
                        </div>
                        <DayPicker
                            mode="range"
